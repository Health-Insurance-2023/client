...
function Main() {
  ...
  const getAllRegistrationForm = useQuery({
    queryKey: [`get-all-form-by-user`, pageNumber, size],
    queryFn: () => registrationFormApi.getAllByUser(pageNumber, parseInt(size)),
            ...
            <Table className='border-spacing-y-[10px] border-separate -mt-2'>
              ...
              <Table.Tbody>
                {filterList?.map((registrationForm, index) => (
                  <RegFormTableRow registrationForm={registrationForm} key={index} isUser={true} />
                ))}
              </Table.Tbody>
            </Table>
          )}
