...
export const dataPlan: StatusFilterType[] = [
  {
    key: 'all',
    content: 'Tất cả'
  },
  {
    key: 'Essential Protection',
    content: 'Gói Essential Protection'
  },
  {
    key: 'Standard Coverage',
    content: 'Gói Standard Coverage'
  },
  {
    key: 'Advanced Care',
    content: 'Gói Advanced Care'
  },
  {
    key: 'Comprehensive Shield',
    content: 'Gói Comprehensive Shield'
  },
  {
    key: 'Premium Assurance',
    content: 'Gói Premium Assurance'
  },
  {
    key: 'Elite Safeguard',
    content: 'Gói Elite Safeguard'
  }
]
...
function Main() {
  ...
  const getAllContract = useQuery({
    queryKey: [`get-user-contract`, pageNumber, size],
    queryFn: () => insuranceContractApi.getAllContract(pageNumber, parseInt(size)),
    onError: (err) => console.log(err)
  })

  const cancelContractMutation = useMutation({
    mutationFn: (id: number) => insuranceContractApi.cancelContract(id),
    onSuccess: () => {
      toast.success('Đã hủy hợp đồng', { autoClose: 2000 })
      getAllContract.refetch()
    },
    onError: (error) => {
  ...
  const cancelContract = (id: number) => {
    cancelContractMutation.mutate(id)
  }

  const userContractList = getAllContract.data?.data.content as ContractType[]
  const totalPage = getAllContract.data?.data.totalPages as number
  ...
  }, [status, plan, getAllContract.isLoading, userContractList, search])

  // console.log(search)

  if (getAllContract.isLoading) return <div>Loading...</div>
