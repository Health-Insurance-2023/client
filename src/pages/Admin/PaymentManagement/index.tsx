...
import AdminPaymentTableRow from 'src/components/AdminPaymentTableRow'
...
function Main() {
  ...
  const getAllPayment = useQuery({
    queryKey: [`get-all-payment`, pageNumber, size],
    queryFn: () => paymentApi.getAllPayment(pageNumber, parseInt(size)),
  ...
  const userPaymentList = getAllPayment.data?.data.content as PaymentType[]
  const totalPage = getAllPayment.data?.data.totalPages as number
  ...
  }, [status, plan, getAllPayment.isLoading, userPaymentList, search, daterange])

  if (getAllPayment.isLoading) return <div>Loading...</div>
            ...
            <Table className='border-spacing-y-[10px] border-separate -mt-2'>
              ...
              <Table.Tbody>
                {filterList?.map((userPayment, index) => (
                  <AdminPaymentTableRow userPayment={userPayment} key={index} payBill={payBill} />
                ))}
              </Table.Tbody>
            </Table>
          )}
