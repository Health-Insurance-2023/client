...
function Main() {
  ...
  const getUserClaim = useQuery({
    queryKey: [`get-all-claim`, pageNumber, size],
    queryFn: () => claimApi.getAllClaim(pageNumber, parseInt(size)),
  ...
  const approveClaimMutation = useMutation({
    mutationFn: (id: number) => claimApi.approveClaim(id),
    onSuccess: () => {
      toast.success('Đã duyệt yêu cầu', { autoClose: 2000 })
      getUserClaim.refetch()
    },
    onError: (error) => {
  ...
  const refuseClaimMutation = useMutation({
    mutationFn: (id: number) => claimApi.refuseClaim(id),
    onSuccess: () => {
      toast.success('Đã từ chối yêu cầu', { autoClose: 2000 })
      getUserClaim.refetch()
    },
    onError: (error) => {
      if (isAxiosBadRequestError<ErrorResponse>(error)) {
  ...
  const refuseClaim = (id: number) => {
    refuseClaimMutation.mutate(id)
  }
